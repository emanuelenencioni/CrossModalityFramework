...
def build_from_config(data_loader, cfg: Dict[str, Any]):
    ...
    if cfg.get('dual_modality', True):
        ...
        eval1 = _build_unimodal_dataset(dataset_type, data_loader[0], img_size=(in_size1, in_size1), confthre=confthre, nmsthre=nmsthre, num_classes=cfg['dataset']['bb_num_classes'], device=cfg['device'])
        eval2 = _build_unimodal_dataset(dataset_type, data_loader[1], img_size=(in_size2, in_size2), confthre=confthre, nmsthre=nmsthre, num_classes=cfg['dataset']['bb_num_classes'], device=cfg['device'])
        return eval1, eval2
    else:
        in_size = cfg['model']['input_size'] if 'input_size' in cfg['model'].keys() else 512
        return _build_unimodal_dataset(dataset_type, data_loader, img_size=(in_size, in_size), confthre=confthre, nmsthre=nmsthre, num_classes=cfg['dataset']['bb_num_classes'], device=cfg['device'])
        



def _build_unimodal_dataset(dataset_type, data_loader, img_size, confthre, nmsthre, num_classes, device):
